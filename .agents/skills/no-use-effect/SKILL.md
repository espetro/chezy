...
## Naming useEffect Functions

When you must use `useEffect` (via `useMountEffect` or in custom hooks), always use a named function expression instead of an anonymous arrow function.

```typescript
// BAD: Anonymous arrow - tells you nothing about intent
useEffect(() => {
  document.title = `${count} items`;
}, [count]);

// GOOD: Named function - reveals purpose at a glance
useEffect(function updateDocumentTitle() {
  document.title = `${count} items`;
}, [count]);
```

### Why Name Your Effects?

1. **Readability**: Four `useEffect(() => {` calls require reading every line to understand intent. Named functions let you skim: `connectToInventoryWebSocket`, `fetchInitialStock`, `resetStockOnLocationChange`, `notifyParentOfStockUpdate`.

2. **Debugging**: Anonymous arrows show `at (anonymous) @ Component.tsx:14` in stack traces. Named functions give you `at connectToInventoryWebSocket @ Component.tsx:14`—critical when triaging errors in production monitoring.

3. **Reveals too much responsibility**: If you struggle to name an effect without "and" or "also" (e.g., `syncWidthAndApplyTheme`), split it into separate effects.

4. **Reveals effects that shouldn't exist**: Vague names like `updateStateBasedOnOtherState` or `syncDerivedValue` signal the code belongs elsewhere (derive inline, use event handlers, etc.).

### Naming Cleanup Functions

Name cleanup returns when teardown is non-trivial:

```typescript
useEffect(function pollServerForUpdates() {
  const intervalId = setInterval(() => {
    fetch(`/api/status/${serverId}`).then(r => r.json()).then(setStatus);
  }, 5000);
...
The symmetry between `pollServerForUpdates` and `stopPollingServer` makes both halves immediately clear.

### Applies Beyond useEffect

Use named functions for `useCallback`, `useMemo`, and reducers:

```typescript
useCallback(function handleFormSubmit() { ... }, [deps]);
useMemo(function computeVisibleItems() { ... }, [deps]);
```

Reference: [Start naming your useEffect functions, you will thank me later](https://neciudan.dev/name-your-effects)
