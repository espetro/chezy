...
// When chezy-mock-data/enriched/listing_outdoor_space.jsonl exists (written by
// `uv run scraper aggregate-outdoor-space`), it fills `outdoor_space` for records
...
const { sidecar, failures: sidecarFailures } = existsSync(OUTDOOR_SPACE_SIDECAR)
  ? parseOutdoorSpaceSidecar(readFileSync(OUTDOOR_SPACE_SIDECAR, "utf8").split("\n"))
  : { sidecar: new Map(), failures: [] };
for (const failure of sidecarFailures) {
  console.error(`skipped sidecar row: ${failure}`);
}
...
for await (const line of rl) {
  ...
  batch.push(toListingRow(mergeOutdoorSpace(parsed.output, sidecar)));
  if (batch.length >= 50) {
    await flush();
  }
}
...
console.log(
  `seeded ${seeded} listings (${failures.length} skipped, ${sidecar.size} outdoor_space from sidecar)`,
);
