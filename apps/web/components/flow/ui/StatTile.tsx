...
export const FlowStatTile = <T extends number>({
  ...
  const stepButton = (target: T | undefined, glyph: React.ReactNode, name: string) => (
    <button
      ...
      className={cn(
        "flex size-7 items-center justify-center rounded-full bg-snow text-steel transition-colors duration-200 hover:text-obsidian disabled:opacity-30 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-obsidian",
      )}
    >
      {glyph}
    </button>
